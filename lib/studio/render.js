...
<!-- Tell email clients we authored for THIS scheme — prevents most clients
     from auto-inverting our colors (Apple Mail, iOS Mail, newer Outlook).
     'light only' for cream-bg brands; 'dark only' for espresso-bg brands. -->
<meta name="color-scheme" content="${brand.colorScheme === 'dark' ? 'dark only' : 'light only'}">
<meta name="supported-color-schemes" content="${brand.colorScheme === 'dark' ? 'dark only' : 'light only'}">

<!--[if mso]><style>* { font-family: Georgia, serif !important; }</style><![endif]-->

<style>
  :root {
    color-scheme: ${brand.colorScheme === 'dark' ? 'dark only' : 'light only'};
    supported-color-schemes: ${brand.colorScheme === 'dark' ? 'dark only' : 'light only'};
  }
