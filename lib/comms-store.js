...
// ── Per-kind enabled/disabled flag ────────────────────────────────────
// Stored as `<prefix>comms:<kind>:disabled` STRING — '1' = paused, absent = active.
// Absence-means-enabled keeps "default on" semantics so new clients get all the
// automated emails firing without any setup.
export async function isCommsDisabled(kind) {
  if (!REDIS_URL || !REDIS_TOKEN) return false;
  try {
    const res = await redis([['GET', `${PREFIX}${kind}:disabled`]]);
    return res?.[0]?.result === '1';
  } catch (_) { return false; }
}
export async function setCommsDisabled(kind, disabled) {
  if (!REDIS_URL || !REDIS_TOKEN) throw new Error('no_redis');
  if (!COMMS_DEFAULTS[kind]) throw new Error('unknown_kind');
  if (disabled) {
    await redis([['SET', `${PREFIX}${kind}:disabled`, '1']]);
  } else {
    await redis([['DEL', `${PREFIX}${kind}:disabled`]]);
  }
  return { ok: true, kind, disabled: !!disabled };
}
