...
function paragraphs(text, brand) {
  ...
  return lines.map(p =>
    `<p style="font-size:16px;line-height:1.75;color:${brand.textSoft};margin:0 0 14px;">${richInline(p).replace(/\n/g, '<br>')}</p>`
  ).join('\n');
}
...
const RENDERERS = {
  devotional(fields, brand) {
    ...
    return `
      ...
      <tr><td style="padding:6px 48px 4px;">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:rgba(16,185,129,0.045);border:1px solid ${brand.borderSoft};border-left:3px solid ${brand.accent};">
  ...
  recap(fields, brand, dataCtx = {}) {
    ...
    if (!posts.length) {
      return `
        ...
        <tr><td style="padding:6px 48px 8px;">
          <p style="font-size:14px;color:${brand.muted};margin:0;font-style:italic;">No recap items this week.</p>
        </td></tr>
      `;
    }
  ...
  giveaway(fields, brand) {
    ...
    return `
      ...
      <tr><td style="padding:6px 48px 8px;">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:rgba(16,185,129,0.06);border:1px solid ${brand.accent}40;">
  ...
  freeletter(fields, brand) {
    const body = paragraphs(fields.body, brand);
    return `<tr><td style="padding:36px 48px 8px;">${body}</td></tr>`;
  },
};
...
<div style="background:radial-gradient(ellipse 800px 500px at 50% 0%, rgba(16,185,129,0.06) 0%, transparent 60%), ${brand.bg};padding:40px 16px;">
