...
// Merge a {presetId, displayFontId, bodyFontId} override on top of CLIENT_BRAND.
// Returns the full effective brand object that templates use at render time.
...
export async function saveOverrides(over) {
  ...
  const clean = {
    presetId:      over?.presetId      || null,
    displayFontId: over?.displayFontId || null,
    bodyFontId:    over?.bodyFontId    || null,
    updatedAt:     Date.now(),
  };
