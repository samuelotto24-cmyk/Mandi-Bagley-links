...
function paragraphs(text, brand, override = '') {
  ...
  return lines.map(p =>
    `<p style="font-size:16px;line-height:1.75;color:${brand.textSoft};margin:0 0 14px${override}">${linkify(richInline(p).replace(/\n/g, '<br>'), brand)}</p>`
  ).join('\n');
}
  ...
  realtalk(fields, brand) {
    ...
    return `
      ...
      ${topic ? `<tr><td style="padding:0 48px 12px;">
        <h2 style="font-family:${brand.displayFont};font-weight:500;font-size:30px;color:${brand.text};margin:0 0 4px;line-height:1.15;letter-spacing:-0.01em${topicStyle}">${topic}</h2>
      </td></tr>` : ''}
...
<table role="presentation" align="center" cellpadding="0" cellspacing="0" border="0" width="600" style="max-width:600px;width:100%;background:linear-gradient(180deg, ${brand.surface} 0%, ${brand.surfaceAlt} 100%);border:1px solid ${brand.border};box-shadow:0 30px 80px rgba(0,0,0,0.10);">
  ...
  <tr><td style="padding:0 48px 28px;text-align:center;">
    ${brand.hideWebsiteLink ? '' : `<a href="${brand.siteUrl}" style="font-size:11px;letter-spacing:0.18em;text-transform:uppercase;color:${brand.accent};text-decoration:none;">${esc(brand.domain || '')}</a>`}
    <div style="font-size:11px;color:${brand.muted};margin-top:${brand.hideWebsiteLink ? '4' : '14'}px;">
